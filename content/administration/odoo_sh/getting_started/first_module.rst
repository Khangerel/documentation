...
Indeed, the purpose of the staging build is to test the behavior of your changes as it would be on
your production, and on your production you would not like your module to be installed
...
.. note::
   - It is not possible to install or upgrade system packages in an Odoo.sh instance (e.g. apt
     packages). However, under `specific conditions,
     <https://www.odoo.sh/faq#install_dependencies>`_ packages can be considered for installation.
     This also applies to **Python modules** requiring system packages for their compilation, and
     **third-party Odoo modules** (`incompatibles modules
     <https://www.odoo.sh/faq#install_dependencies>`_).
   - **PostgreSQL extensions** are not supported by Odoo.
...
The feature is explained in this section by using the
`Unidecode library <https://pypi.python.org/pypi/Unidecode>`_ in your module.
...
.. code-block:: bash
  ...
  $ git add my_module
  $ git commit -m "[IMP] my_module: automatically remove special chars in my_module.my_module name
    field"
